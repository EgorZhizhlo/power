...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const dayLimitValue = formData.get('day_limit');

    const jsonData = {
      name: formData.get('name'),
      day_limit: (dayLimitValue && dayLimitValue.trim() !== '') ? parseInt(dayLimitValue) : null,
    ...
    try {
      ...
      if (response.status === 204) {
        window.location.href = `/companies/routes?company_id=${window.companyId}`;
