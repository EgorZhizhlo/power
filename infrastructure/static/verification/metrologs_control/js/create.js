...
function calculateError() {
  const meterFields = document.querySelectorAll('[name^="first_meter_water_according_"], [name^="second_meter_water_according_"], [name^="third_meter_water_according_"]');
  const referenceFields = document.querySelectorAll('[name^="first_reference_water_according_"], [name^="second_reference_water_according_"], [name^="third_reference_water_according_"]');
  const resultFields = document.querySelectorAll('[name^="first_water_count_"], [name^="second_water_count_"], [name^="third_water_count_"]');
  for (let i = 0; i < meterFields.length; i++) {
    ...
    resultFields[i].value = referenceValue ? (((meterValue - referenceValue) / referenceValue) * 100).toFixed(2) : '';
  }
  validateError();
}
...
function validateErrorForRange(range, limit) {
  const fields = document.querySelectorAll(`[name^="first_water_count_${range}"], [name^="second_water_count_${range}"], [name^="third_water_count_${range}"]`);
  let invalid = false;
...
function initFormSubmit() {
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      const res = await fetch(`/verification/api/metrologs-control/create/?${params.toString()}`, {
        method: 'POST',
